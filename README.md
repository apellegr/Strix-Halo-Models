...
|--------|-------|-----------|--------|
| `start-qwen3-235b.sh` | Qwen3-235B Thinking (Q3_K_M) | 12.8 tok/s | ~88 GB |
| `start-qwen35-122b.sh` | Qwen 3.5 122B-A10B (MXFP4) | MoE, 10B active | ~70 GB |
| `start-gpt-oss-120b.sh` | GPT-OSS 120B (MXFP4) | 51 tok/s | ~63 GB |
| `start-coder-32b.sh` | Qwen 2.5 Coder 32B (Q4_K_M) | 10.5 tok/s | ~24 GB |
...
│   └── ROCM-MEMORY-LIMIT-INVESTIGATION.md  # GPU memory deep-dive
...
```bash
./start-llm-server.sh stop qwen3-235b-thinking  # Stop specific model
./start-llm-server.sh stop                       # Stop all
./start-llm-server.sh logs qwen3-235b-thinking   # View logs
...
```bash
./benchmark-model.sh <model-name> --optimize  # Find working GPU layers
```

### Slow Performance
1. Check GPU offload: `./start-llm-server.sh logs <model-name> | grep "offload"`
2. Verify ROCm: `rocm-smi`
3. Re-optimize: `./benchmark-model.sh <model-name> --optimize`
